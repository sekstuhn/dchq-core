class DelayedJob::Import::RentalProduct < Struct.new(:data, :store, :user, :params)
  def perform
    errors = []
    Product.transaction do
      data.each_with_index do |record, index|
        ...
        @rp = store.rental_products.build
        @rp.build_logo

        import( record )
        unless @rp.save
          errors << "#{I18n.t("controllers.on_line")} #{index + 1}: #{@rp.errors.full_messages.join(", ")}"
        end
      end
    end
    if errors.blank?
      ImportMailer.success(user, I18n.t('activerecord.models.rental_product.one')).deliver
    else
      ImportMailer.rejected(user, I18n.t('activerecord.models.rental_product.one'), errors).deliver
    end
  end
  ...
  def import record
    record.each_with_index do |item, i|
      ...
      when "brand_id" then @rp[params["fields_#{i + 1}"]]           = find_or_create_brand( item )
      when "category_id" then @rp[params["fields_#{i + 1}"]]        = find_or_create_category( item )
      when "supplier_id" then @rp[params["fields_#{i + 1}"]]        = find_or_create_supplier( item )
      when "tax_rate_id" then @rp[params["fields_#{i + 1}"]]        = find_or_create_tax_rate( item )
      when "commission_rate_id" then @rp[params["fields_#{i + 1}"]] = find_or_create_commission_rate( item )
      when "image" then @rp.logo.upload_image_from_url item
      else @rp[params["fields_#{i + 1}"]] = item.strip
